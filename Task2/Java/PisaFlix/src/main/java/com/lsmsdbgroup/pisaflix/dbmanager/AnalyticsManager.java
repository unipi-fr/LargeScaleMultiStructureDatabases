...
public class AnalyticsManager implements AnalyticsManagerDatabaseInterface {
    ...
    public Set<EngageResult> engagementAnalytics(Date startDate, Date endDate, Entity entity) {
        ...
        film.getCommentSet().stream().map((comment) -> (Date) comment.getTimestamp()).filter((date) -> !(date.getTime() < startDate.getTime() || date.getTime() > endDate.getTime())).map((date) -> {
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(date);
            return calendar;
        }).map((calendar) -> calendar.get(Calendar.YEAR)).forEachOrdered((year) -> {
